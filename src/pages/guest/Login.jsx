  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-luxora-bg px-4 relative overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="card w-full max-w-md p-8 relative z-10">
        ...
        <h2 className="text-lg font-semibold text-luxora-text mb-1">Welcome Back</h2>
        <p className="text-sm text-luxora-muted mb-6">Sign in to manage your reservations and stays.</p>
        ...
        <div className="mt-6 pt-4 border-t border-luxora-border flex flex-col gap-2">
          <button
            type="button"
            onClick={() => { setEmail('guest@luxora.com'); setPassword('Password123!'); }}
            className="w-full py-2 px-3 rounded-xl border border-luxora-gold/30 bg-luxora-gold/5 text-xs text-luxora-gold font-medium hover:bg-luxora-gold/15 transition-colors"
          >
            ⚡ Quick Fill Demo Guest Credentials
          </button>
        </div>

        <p className="text-center text-sm text-luxora-muted mt-5">
          New to Luxora? <Link to="/account/signup" className="text-luxora-gold font-medium hover:underline">Create an account</Link>
        </p>
      </motion.div>
    </div>
  );
